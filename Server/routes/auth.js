...
var auth = {
    ...
    validate: function(username, password,callback) {
        ...
        userSchema.findOne({$or: [{username: username}, {email: username}]},{_id:0,userId:1,username:1,password:1,gender:1,postsCounts:1,blockList:1,interestCategories:1,favouriteProfiles:1,profilePictureSet:1,profilePictureUrls:1,followings:1,privacy:1,emailVerified:1,rate:1,phoneVerified:1},function(err,user) {
            if (err) throw err;
            if (user) {
                let hashPassword = CryptoJS.HmacSHA512(password, user.userId).toString();
